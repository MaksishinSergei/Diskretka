...
{
    bool l = imp((x || !y), (!(eqv(z, !x)))); //вводимаая функция
    return l;
}
...
int main()
{
    ...
    bool x, y, z, fun;
    cout << endl;
    cout << setw(3) << "x" << setw(3) << "y"<< setw(3)<< "z" << setw(3) << "f" << setw(3) << endl;
    cout << endl;
    for (int i = 0; i < 8; i++) {
        if (i < 4) {
            x = 0;
        ...
        cout << setw(3) << x << setw(3) << y << setw(3) << z << setw(3) << f(x, y, z) << setw(3) << endl;
        fun = f(x, y, z);
        if (fun == 1) {
